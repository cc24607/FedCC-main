...
model_list = ['CNN_emnist', 'CNN_mnist', 'CNN_fashion_mnist', 'CNN_cifar10', 'ResNet18', 'ShuffleNet', 'MobileNet',
              'LSTM']
dataset_list = ['emnist', 'mnist', 'fashion_mnist', 'cifar10', 'cifar100', 'IMDB']
method_list = ['Fedavg', 'moon', 'FedUV', 'FedDistill+', 'Fedprox', 'Fedproto', 'Fedrep', 'FedBABU', 'FedDW',
               'FedDW(pFL)', 'Fedper', 'Per-Fedavg', 'FedALA', 'local_only']

# Fedavy: 0  moon: 1  FedUV: 2  FedDistill+: 3  Fedprox: 4  Fedproto: 5  Fedrep: 6  FedBABU: 7  FedDW: 8
# FedDW(pFL): 9  Fedper: 10  FedALA: 12  loacl_only: 13  cifar100使用ResNet18网络

class Configs:
    def __init__(self):
        self.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        self.learning_rate = 0.001
        self.server_epoch = 50
        self.client_epoch = 5
        self.n_client = 10
        self.batch_size = 128
        self.participation_rate = 0.5
        self.idd = 0
        self.dirichlet_alpha = 0.6 #异构系数

        self.id_dataset = 3
        self.id_model = 3
        self.id_method = 1
        self.save_model_path ='./models/' + 'alpha='+ str(self.dirichlet_alpha)+'-' + method_list[self.id_method] + '-' + model_list[self.id_model] + '-' + \
                               dataset_list[self.id_dataset] + '.pkl'
