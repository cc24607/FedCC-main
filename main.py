...
if __name__=='__main__':
    ...
    if config.name_method == 'Fedavg':
        ...
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'Fedavg_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        ...
        print("moon")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'moon_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for epoch in range(config.server_epoch):
            print("epoch : ", epoch+1)
            local_ws = []
            sum_choose=0
        ...
        print("Feduv")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedUV_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for epoch in range(config.server_epoch):
            print("epoch : ", epoch+1)
            local_ws = []
            global_model.train()
        ...
        print("FedDistill+")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedDistill+_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        ...
        print("Fedprox")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'Fedprox_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for epoch in range(config.server_epoch):
            print("epoch : ", epoch+1)
            local_ws = []
            sum_choose=0
        ...
        print("Fedproto")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'Fedproto_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        ...
        print("Fedrep")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'Fedrep_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for client in list_client:
            client.model=copy.deepcopy(global_model).to(config.device)
            pass
        ...
        print("Fedper")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'Fedper_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for client in list_client:
            client.model=copy.deepcopy(global_model).to(config.device)
            pass
        avg_w=global_model.state_dict()
        ...
        print("FedBABU")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedBABU_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        ...
        print("FedDW")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedDW_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        distills = torch.eye(config.n_class).to(config.device)
        eye = torch.eye(config.n_class).to(config.device)
        # distills = torch.zeros(config.n_class,config.n_class).to(config.device)
        client_distill = []
        ...
        print("FedDW(pFL)")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedDW(pFL)_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        distills = torch.eye(config.n_class).to(config.device)
        client_distill = []
        client_counts = []
        for client in list_client:
        ...
        print("Per-Fedavg")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'PerFedavg_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for epoch in range(config.server_epoch):
            print("epoch : ", epoch+1)
            local_ws = []
            sum_choose=0
        ...
        print("FedALA")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'FedALA_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        ...
        print("local_only")
        save_path = 'alpha=' + str(
            config.dirichlet_alpha) + '-' + 'local_only_acc' + '-' + config.name_model + '-' + config.name_dataset + '.json'
        for client in list_client:
            client.model = copy.deepcopy(global_model).to(config.device)
            pass
        for epoch in range(config.server_epoch):
    ...
    # 保存模型文件到本地
    torch.save(global_model.state_dict(), config.save_model_path)
    # 保存ep_list和acc_list这两个列表的对应关系到本地
    data = {'epochs': ep_list, 'accuracies': acc_list}
    output_dir = './results'
    os.makedirs(output_dir, exist_ok=True)
    file_path = os.path.join(output_dir,save_path)
